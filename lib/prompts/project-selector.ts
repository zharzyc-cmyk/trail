  ...
  "sectionPlans": [
    {
      ...
      "useProjects": [],
      "instruction": "3 条最贴 JD 的卖点，每条带能力名 + 简短佐证，**严格 ≤40 字**"
    },
    {
      "title": "教育背景",
      "maxBullets": 2,
    ...
    },
    {
      ...
      "instruction": "**只 2 段**实习（最贴 JD 的 2 段，其余进 excludedProjects），每段 **3 bullet**，每条 ≤60 字。**只能用 useProjects 里的项目**"
    },
    {
      "title": "项目经历",
      "maxBullets": 3,
      ...
      "instruction": "1 个独立项目（比赛/论文/独立 AI 开发），3 bullet × ≤60 字。**禁止包含实习公司**。如项目库都是实习类，直接 skip:true",
      "skip": false
    },
    {
      "title": "专业技能",
      ...
      "useProjects": [],
      "instruction": "工具/语言/证书清单，分类紧凑 ≤50 字/行。**禁止重复核心能力的能力陈述**"
    }
  ]
}
\`\`\`
...
1 页 A4 含 header、章节标题、bullet 文字，**总字符上限约 1050**（实测：超 1100 字就开始溢出第 2 页，浏览器打印对话框默认页眉页脚还会再吞 1 行）。

### 推荐配额（按 1050 总预算反推，留 100 字余量）

- **核心能力**：3 条 × ≤40 字 = ~120 字
- **教育背景**：2 条 × ≤60 字 = ~120 字
- **实习经历**：2 段 × **3 bullet** × ≤60 字 = ~360 字
- **项目经历**：1 项目 × **3 bullet** × ≤60 字 = ~180 字（如果项目库都是实习类，直接 skip 整段）
- **专业技能**：4 行 × ≤50 字 = ~200 字
...
加总 \`sum(maxBullets × maxCharsPerBullet)\` 所有非 skip section。**必须 ≤ 980**（留 70 字给 header）。

- ≤ 980 → 提交
- 980-1100 → **回去砍**（优先：项目经历 skip / 实习经历降到 1 段 / bullet 数 3 → 2）
- > 1100 → **强制 skip 项目经历** + 实习经历每段 2 bullet

**不允许提交超出 980 的方案。** 宁短不超。
