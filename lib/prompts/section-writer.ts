...
2. **单条 bullet 硬上限 = maxCharsPerBullet 个中文字（**字符数自检：写完一条数一遍**）**。
   - 一条 bullet 不是一个完整段落，是**一句话**。
   - 如果你想说的内容超过 maxCharsPerBullet，**砍掉所有修饰语**：副词、过渡词、形容词、"成功 / 高效 / 通过 / 凭借 / 不仅 / 而且"等填充词全部删除，只保留"动词 + 对象 + 数据"。
   - **目标格式**："[bp]小标题：动词 + 对象，数据 N。" 一气呵成不拖沓。
   - 反面例子（错的）：\`<li><span class="bp">数据分析</span>：在团队漏斗看板的基础上对内容表现进行持续追踪与分析，制定自然流量达阈值才投放的标准，将有限预算集中在已验证内容，体现数据分析与 ROI 意识。</li>\`（80 字，超）
   - 正面例子（对的）：\`<li><span class="bp">数据驱动投流</span>：基于漏斗看板设"流量达阈才投"标准，投放 ROI 提升 <strong>30%</strong>。</li>\`（38 字 ✓）
...
5. **用户痛点提醒**：用户多次抱怨简历"溢出 2 页 / 必须自己手动删"。每多 10 字就多 1mm 高度，5 条 bullet 多 50 字就让简历溢页。**你的每条 bullet 越短，用户越满意**。
